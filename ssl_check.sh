...
echo "ssl_check.sh running..." >> "$LOG_FILE"
# Logging only the relevant information
if [ -z "$expiry_date" ]; then
    echo "Could not retrieve SSL certificate for $URL" >> "$LOG_FILE"
else
    echo "SSL certificate for https://$URL expires on: $expiry_date" >> "$LOG_FILE"
fi

if [ -z "$expiry_date_test" ]; then
    echo "Could not retrieve SSL certificate for $TEST_URL" >> "$LOG_FILE"
else
    echo "SSL certificate for https://$TEST_URL expires on: $expiry_date_test" >> "$LOG_FILE"
fi